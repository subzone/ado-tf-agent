...
|---|---|---|---|
| S3 Public Access | 🔴 High | `aws_s3_bucket_public_access_block` | Any block flag is `false` |
| Open Ingress SSH/RDP | 🔴 High | `aws_security_group` | Port 22 or 3389 open to `0.0.0.0/0` |
| RDS Unencrypted | 🔴 High | `aws_db_instance` | `storage_encrypted` is not `true` |
...
| Open Ingress (other) | 🟡 Medium | `aws_security_group` | Any port open to `0.0.0.0/0` |
| IMDSv2 Not Enforced | 🟡 Medium | `aws_launch_template` | `http_tokens` is not `required` |
| EBS Unencrypted | 🟡 Medium | `aws_ebs_volume` | `encrypted` is not `true` |
...
| Rule | Severity | Resource type | Condition |
|---|---|---|---|
| Blob Public Access | 🔴 High | `azurerm_storage_account` | `allow_blob_public_access` is `true` |
| HTTP Allowed | 🔴 High | `azurerm_storage_account` | `enable_https_traffic_only` is `false` |
